...
export class JohtoComponent implements OnInit, OnDestroy {
  pokemonJohtoArray: PokeModel[] = [];
  userInput: string = '';
  apiUrl: string = 'https://pokeapi.co/api/v2/pokemon/?limit=251';
  ...
  ngOnInit() {
    this.pokemonSubscription = this.http.get(this.apiUrl).subscribe(
      (res) => {
         ...
         this.pokemonJohtoArray = resJSON.results.filter((item: { name: string; url: string; }, index: number) => {
            if (index > 150) {
              return new PokeModel(item.name, item.url);
            }
          });
          
         for(const item of this.pokemonJohtoArray) {
  ...
  open(index: number) {
    ...
    modalRef.componentInstance.pokeDetail = this.pokemonJohtoArray[index];
  }
